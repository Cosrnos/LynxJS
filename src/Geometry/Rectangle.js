...
Lynx.Rectangle = function (pX, pY, pWidth, pHeight) {
	...
	that.Contains = (function (pPoint) {
		return (pPoint.X >= this.Points.BottomLeft.X && pPoint.Y >= this.Points.BottomLeft.Y && pPoint.X <= this.Points.TopRight.X && pPoint.Y <= this.Point.TopRight.Y);
	}).bind(that);

	return that;
};

Lynx.Rect = function (pX, pY, pWidth, pHeight) {
	return Lynx.Rectangle(pX, pY, pWidth, pHeight);
};
